...
import { ArrowRight, Bolt, Target, TimerReset, TrendingUp } from "lucide-react";
import { NetworkBackground } from "./network-background";
import { SxButton } from "./sx-button";
import { Counter } from "./counter";

const words = ["Hire", "AI", "Employees", "for", "Every", "Team."];
  ...
  return (
    <section ref={ref} className="relative overflow-hidden pb-20 pt-36 md:pb-28 md:pt-44">
      ...
      >
        ...
        <h1 className="mx-auto mt-8 max-w-[900px] text-[2.6rem] font-extrabold leading-[0.95] tracking-[-0.04em] md:text-[5rem]">
          {words.map((w, i) => (
            <motion.span
              key={w + i}
              ...
              className={
                w === "AI" || w === "Employees" ? "mr-[0.28em] inline-block text-electric" : "mr-[0.28em] inline-block"
              }
            >
              {w}
            </motion.span>
        ...
        <motion.p
          ...
          className="mx-auto mt-7 max-w-[650px] text-[1.1rem] leading-[1.7] text-muted-foreground md:text-[1.25rem]"
        >
        ...
        >
          {[
            ...
            const Icon = s.icon;
            return (
              <div key={i} className="rounded-2xl border border-border/70 bg-white/70 px-4 py-4 shadow-soft backdrop-blur-sm">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-electric-soft text-electric">
                  <Icon className="h-4 w-4" />
                </div>
                <dd className="mt-4 font-display text-2xl font-semibold text-navy md:text-3xl">{s.v}</dd>
