        ...
        {plans.map((p, i) => (
          <Reveal key={p.name} delay={i * 0.08}>
            ...
            >
              ...
              <p className={cn("mt-2 text-sm", p.highlighted ? "text-white/70" : "text-muted-foreground")}>
                {p.tagline}
              </p>
              ...
              <div className="mt-8 flex items-baseline gap-1.5">
                {p.price ? (
                  <>
                    ...
                    >
                      ${annual ? Math.round(p.annual! / 12).toLocaleString() : p.price.toLocaleString()}
                    </span>
                    <span
                      className={cn("text-sm", p.highlighted ? "text-white/60" : "text-muted-foreground")}
                    >
                      /month
                    </span>
                  </>
